...
\item Express the vector $\overrightarrow{QP}$ in terms of $s$ and $\phi$. %\emph{Hint:} First express the length of this vector in terms of $t$ and $\phi$, and then in terms of $a$, $\theta$, and $\phi$.
\[
         \overrightarrow{QP} = \left( s \sin\phi \right) \langle 0,0,1  \rangle
\]
...
\[
     \theta = \frac{t}{a} = \frac{s \cos\phi}{a}
\]
