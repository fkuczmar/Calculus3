...
\begin{question}  \label{Qdf5r4r54tr}

The function
\[
   {\bf p}(t) = \biggr \langle  \, 3\cos\left( 2t-4 \right), 4 \ln (t/2) , e^{3(t-2)} \biggr \rangle , \, 1 \leq t \leq 4 ,
\] 
expresses the position of a fly relative to the origin (in meters) in terms of the number of seconds past noon.

\begin{enumerate}
\item Find the fly's speed at time $t=2$ seconds past noon.

\item Is distance between the fly and the point $(1,2,4)$ increasing or decreasing at this time? At what rate?

\item Is the fly speeding up or slowing down at this time? At what rate?
...
\begin{explanation}
\begin{enumerate}
\item The velocity of the fly at time $t$ seconds past noon is
\begin{align*}
   {\bf v} (t) & = \frac{d}{dt}({\bf p}(t)) \\
               & = \biggr \langle -6\sin(2t-4) , \frac{4}{t} , 3e^{3(t-2)}  \biggr \rangle 
\end{align*}

So at time $t=2$, the fly has velocity 
\[
    {\bf v}(2) = \langle 0 , 2, 3  \rangle \text{ m/s} 
\]
and speed
\[
 |{\bf v}(2)| = \sqrt{13} \text{ m/s} .
\]

\item The rate of change in the distance between the fly and the point $(1,2,4)$ is equal to the derivative
\begin{align*}
   \frac{d}{dt} \left| {\bf p}(t) - \langle 1,2,4 \rangle) \right| &= \frac{d}{dt} \left( \sqrt{({\bf p}(t) - \langle 1,2,4 \rangle)\cdot({\bf p}(t) - \langle 1,2,4 \rangle)} \right)   \\ \\
               &= \frac{{\bf v}(t)\cdot({\bf p}(t) - \langle 1,2,4 \rangle)}{|{\bf p}(t) - \langle 1,2,4 \rangle|} .
\end{align*}
This is the scalar projection of the fly's velocity vector in the direction of the fly's position relative to the point $(1,2,4)$. 

At time $t=2$, the fly has position
\[
 {\bf p}(2) = \langle 3, 0, 1\rangle \text{ meters}
\]
relative to the origin and position
\[
    \langle 3, 0, 1\rangle \text{ m} - \langle 1,2,4\rangle \text{ m}  = \langle 2, -2, -3 \rangle \text{ m}
\]
relative to $(1,2,4)$ .

So at time $t=2$ seconds past noon, the distance between the fly and the point $(1,2,4)$ is changing at the rate of
\begin{align*}
     \langle 0,2,3 \rangle \text{ m/s} \cdot \left(\frac{1}{\sqrt{17}}\langle 2,-2,-3\rangle\right)   = -\frac{13}{\sqrt{17}} \text{ m/s} .
\end{align*}
So at time $t=2$ the distance between the fly and the point $(1,2,4)$ is decreasing at the rate of 
\[
 \frac{13}{\sqrt{17}} \text{ m/s} .
\]


\item The rate of change in the fly's speed with respect to time is equal to the derivative
\begin{align*}
   \frac{d}{dt}\left( \sqrt{{\bf v}(t)\cdot {\bf v}(t)} \right) = \frac{{\bf a}(t)\cdot {\bf v}(t)}{|{\bf v}(t)|} .
\end{align*}

This rate of change is the scalar projection of the fly's acceleration in the direction of motion.

Differentiating the velocity vector above with respect to time, gives the acceleration vector 
\[
    {\bf a}(t) = \biggr \langle -12 \cos(2t-4) , -\frac{4}{t^2} , 9e^{3(t-2)} \biggr \rangle 
\]

So at time $t=2$ seconds past noon, the fly has acceleration
\[
  {\bf a}(2) = \langle -12, -1, 9 \rangle \frac{\text{m}}{\text{s}^2} . 
\]
At this time, its speed is increasing at the rate of
\begin{align*}
   \frac{{\bf a}(2)\cdot {\bf v}(2)}{|{\bf v}(2)|} &= \langle -12, -1, 9 \rangle \frac{\text{m}}{\text{s}^2} \cdot \left(\frac{1}{\sqrt{13}}\langle 0,2,3\rangle\right) \\
         & = \frac{25}{\sqrt{13}} \frac{\text{m}}{\text{s}^2} .
\end{align*}
...
\begin{question} \label{QmWMr3490}

Use the first three terms 
\[
   p(x) =  1 + \frac{1}{2}x + \frac{3}{8}x^2
\]
of the Maclaurin series for the function
\[
   f(x) =  \frac{1}{\sqrt{1-x}} 
\]
to find the first three terms of the Maclaurin series for the function
\[
 g(x) = \arcsin x .
\]
...
\]
%we substitute $x^2$ for $x$ in the expression for $p(x)$ to get
%\[
%    p(x^2) = 1 + \frac{1}{2}x^2 + \frac{3}{8}x^4 .
%\]

Since the first three terms of the MacLaurin series for the function 
\[
    f(x) = \frac{1}{\sqrt{1-x}}
\]
are
\[
  p(x) =  1 + \frac{1}{2}x + \frac{3}{8}x^2,
\]
the first three terms of the MacLaurin series for the function
\[
...
So the first three terms of the MacLaurin series for
\[
   \int_0^x \frac{dt}{\sqrt{1-t^2}} = \arcsin x - \arcsin 0
\]
are
\[
   \int_0^x \left( 1 + \frac{1}{2}t^2 + \frac{3}{8}t^4  \right) dt = x + \frac{1}{6}x^3 + \frac{1}{40}x^5 . 
\]

And the first three terms of the MacLaurin series for $\arcsin x$ are
\[
    \arcsin 0 + x + \frac{1}{6}x^3 + \frac{1}{40}x^5 = x + \frac{1}{6}x^3 + \frac{1}{40}x^5.
\]

%\[
%      \frac{d}{dx} \left(\arcsin x \right) = \frac{1}{\sqrt{1-x^2}} \sim p(x^2)  = 1 + \frac{1}{2}x^2 + \frac{3}{8}x^4 .
%\]
%Then 
%\begin{align*}
%\int_0^x \frac{dt}{\sqrt{1-t^2}} & \sim \int_0^x \left( 1 + \frac{1}{2}t^2 + \frac{3}{8}t^4  \right) dt ,
 %\int_0^x \left( 1 + \frac{1}{2}t^2 + \frac{3}{8}t^4  \right) dt &= 1 + \frac{1}{2}x^2 + \frac{3}{8}x^4 \right)
%\end{align*}
%and 
%\[
%      \arcsin x - \arcsin 0 \sim x + \frac{1}{6}x^3 + \frac{1}{40}x^5 . 
%\]
%Since $\arcsin 0 = 0$,  %the first three terms of the MacLaurin series for $\arcsin x$ are
%\[
 % \arcsin x  \sim x + \frac{1}{6}x^3 + \frac{1}{40}x^5 .
%\]
\end{explanation}
...
\begin{question} \label{Q111QWQWQ}
Let 
\[
     z = h(x,y) = 3 - \sqrt{64 - x^2 - 2y^2} .
\] 

\begin{enumerate}
\item Sketch the domain of $h$. Label the axes with the appropriate variable names. Include scales on the axes. Explain your reasoning.

\item Write the domain of $h$ in set-builder notation.

\item Find the range of $h$ and write it in set-builder notation. Explain your reasoning.

\item Find equations of three level sets of the function $h$. Then sketch them below. Label the axes with the appropriate variable names. Include scales on the axes. 

\end{enumerate}

\begin{explanation}
\begin{enumerate}
\item The point $(x,y)$ is in the domain of $f$ if and only if
\[
          64 - x^2 - 2y^2 \geq 0,
\]
...
So the domain is the set of points on or inside the ellipse
\[
       x^2 + 2y^2 = 64.
\]
The ellipse has intercepts $(\pm 8, 0)$ and $(0, \pm \sqrt{32})$ and the domain of $f$ is sketched below.

\begin{onlineOnly}
    \begin{center}
\desmos{eoemyi7rjd}{900}{600}
\end{center}
\end{onlineOnly}

\item The domain of $f$ is the set
\[
 \{ (x,y)\in \mathbb{R}^2 |  x^2 + 2y^2 \leq 64  \}.
\]

\item Since 
\[
     0\leq  64 - x^2 - 2y^2 \leq 64
\]
for $(x,y)$ in the domain of $f$,
\[
  0\leq  \sqrt{64 - x^2 - 2y^2} \leq 8, 
\]
and
\[
   -5 \leq 3 - \sqrt{64 - x^2 - 2y^2} \leq 3 .
\]
So the range of the function
\[
  z = f(x,y) = 3 - \sqrt{64 - x^2 - 2y^2}
\]
is the set
\[
 \{ z \in \mathbb{R} | -5 \leq z \leq 3 \}.
\]

\item The level set for $z=-5$ is the curve
\[
   -5 = 3 - \sqrt{64 - x^2 - 2y^2}
\]
or  equivalently the curve
\[
     x^2 + 2y^2 = 0.
\]
This level set is the origin $(0,0)$.

The level set for $z=3$ is the curve
\[
     3 = 3 - \sqrt{64 - x^2 - 2y^2}
\]
or the ellipse
\[
     x^2 + 2y^2 = 64.
...
The level set for $z=-1$ is the curve
\[
     -1 = 3 - \sqrt{64 - x^2 - 2y^2}.
\]
This is the ellipse
\[
  x^2 + 2y^2 = 48.
