...
To be able to solve this problem in general, we need a way of finding a vector perpendicular to a given vector. We saw in the previous question, that an easy way to do this is to swap the components and negate one of them. Later in the class we'll learn how to do this with vector operations, but for now we'll use desmos to define a function $j$ that acts on a vector ${\bf w}$ and returns the vector we get by rotating ${\bf w}$ by $\pi/2$ radians counterclockwise.
