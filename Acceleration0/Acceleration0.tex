...
\title{The Acceleration Vector, Introduction}
...
\begin{question}  \label{QPfdefer43445r}
Given the velocity and acceleration vectors of a motion at a certain time, either graphically or numerically, how would we determine whether the object is speeding up or slowing down at this instant? And how can we approximate or compute this rate of change of speed with respect to time?

For example, suppose the velocity and acceleration vectors at time $t$ seconds are as illustrated below.  Suppose also that
\[
    |{\bf v}(t)| = 18 \text{ (m/s)} 
\] 
\[
...
\begin{enumerate}
\item Do you think the object is speeding up or slowing down at this time? Why?

\item How would you approximate the rate of change in the speed (with respect to time) at this instant?

\item How would you approximate the object's velocity $0.1$ seconds later? Its speed?
\end{enumerate}

\begin{onlineOnly}
    \begin{center}
\desmos{76rs0m8ofp}{900}{600}
\end{center}
\end{onlineOnly}

\href{https://www.desmos.com/calculator/76rs0m8ofp}{163: Tangential Component of Acceleration}
...
\item The first is to ammend the picture above by activating the folder \emph{Solution} in Line 1 to approximate the change in speed over a small time interval $\Delta t$. Fill in the details to show 
\[
     \frac{d}{dt}\left( |{\bf v}(t)| \right) = \frac{{\bf a}\cdot {\bf v}}{|{\bf v}|} .
\] 
 
In other words, the rate of change of speed with respect to time is the scalar projection of the acceleration vector in the direction of motion (ie. in the direction of the velocity vector). 

\item Use the chain rule and the product rule to compute the derivative
\[
 \frac{d}{dt}\left( |{\bf v}(t)| \right) = \frac{d}{dt} \left( \sqrt{{\bf v}(t) \cdot {\bf v}(t)}   \right) .
\]

\item The idea is to forget about the position function. Instead we'll write the velocity function of a motion
in polar form as
\[
...
The derivative $dv/dt$ is exactly what we want to compute. It is the rate of change of speed with respect to time. To pick off this rate of change we just take the scalar product of the acceleration with the unit vector
\[
...
The advantage of this proof is that it is simple. A disadvantage is that it works only for motions in the plane. The same fact holds for motions in space as the proofs in parts (a) and (b) show.

Next a few comments about acceleration. Then we'll look at some examples.
...
\section{Comments About Acceleration}
...
Let's first clear up two common misconceptions. One is that acceleration measures the rate of change in an object's speed with respect to time. This cannot possibly be true. Acceleration is a vector, but the rate of change of speed with respect to time is a scalar.

The other is that for circular motion the acceleration vector always points directly toward the center of motion. The first example below shows that this is false.

To visualize the acceleration vector we gather the velocity vectors together by pinning their tails at a common origin. The tips of these pinned vectors trace out a curve called the \emph{hodograph} (from the Greek \emph{hodos} for path, literally \emph{path-writer}. other related words include odometer, method, exodus, period, cathode, anode), but it might as well be called the velocity curve. As a point $P$ follows the motion, the tip $P^*$ of the velocity vector traces the hodograph and is a motion itself. The derivative of this new motion, with respect to time, is the acceleration vector. This makes the acceleration vector tangent to the hodograph. And the magnitude of the acceleration is just the speed of $P^*$ as it moves along the hodograph. Here's an example.


\begin{example} \label{EX98df3rhg}
The point $P$ oscillates back and forth on the circular path shown below.
...
\item Drag or play the slider $u$ in Line 2 to start the motion.

\item Look at the velocity vector and the hodograph to make sure you understand how the hodograph is determined.

\item Watch the acceleration vector (green) in the motion and on the hodograph to make sure you understand what's going on.

\item Activate the folder in Line 1 to resolve the accelaration vector into components parallel and normal to the velocity vector.

\item Look at the graphs of the speed function and the rate of change in the speed function. How is the latter related to the acceleration vector? 
...
Play the slider in Line 1 to activate the animation below and see a mass oscillating in simple harmonic motion. Pay particular attention to the velocity and acceleration vectors as wel as the graphs of the speed function (orange) and the function that gives the rate of change of speed with respect to time (red).
...
To better understand the velocity and acceleration vectors, we need to first realize that simple harmonic motion is driven by uniform (constant speed) circular motion. To see what this means activate the folder \emph{Circle} on Line 2 above.

You'll see a point $P$ moving around a circle at a constant speed, along with its velocity and acceleration vectors. Note that these vectors are orthogonal. Explain why.
\begin{freeResponse}
\end{freeResponse}

We get the simple harmonic motion of the mass by projecting the uniform circular motion of $P$ orthogonally onto the horizontal diameter. Then the velocity and acceleration of the mass are the vector projections of the veloicty and acceleration of $P$ in the horizontal direction. Make sure you can see this in the animation.
...
\begin{exercise}  \label{Q34r05r34gtt}
\begin{enumerate}
...
\begin{exercise} \label{ExPRefrer3}

The function
\[
 {\bf p}(t) = \langle 6 \cos (t/2) , 3\sin(t/2) \rangle \, , \, 0\leq t \leq 4\pi, 
\]
gives the position relative to the origin (in cm) of a beetle at time $t$ seconds.

\begin{onlineOnly}
    \begin{center}
\desmos{i9yfwzv3om}{900}{600}
\end{center}
\end{onlineOnly}

\href{https://www.desmos.com/calculator/i9yfwzv3om}{163: Acceleration Vector 2}

\begin{enumerate}
\item Is the beetle slowing down or speeding up as it passes the point $(18/5, 12/5)$cm? At what rate?
\end{enumerate}
...
\begin{exercise} \label{Ex87d3}

\begin{enumerate}

\item The continuous function ${\bf g_1}(t)$, $t\geq 0$, gives the acceleration (in m/s/s) of a rock at time $t$ seconds. The rock has velocity ${\bf v}_2$ at time $t=2$ seconds and position ${\bf p}_2$ relative to the orign at this time.

Find expressions for the rock's velocity ${\bf v}(t)$(in meters/sec) and position ${\bf p}(t)$ (in meters) functions.

\item Suppose now that the acceleration is constant and equal to ${\bf g}$. Use the result of part (a) to find expressions for the veloicty and position functions.

\begin{onlineOnly}
    \begin{center}
\desmosThreeD{ggn3h1fpm1}{900}{600}
\end{center}
\end{onlineOnly}

\href{https://www.desmos.com/3d/ggn3h1fpm1}{163: Projectile Motion 24}

\item Write and solve the appropriate inequality to determine when the rock is slowing down.

\item Suppose as in the animation above that
\[
 {\bf g} = \langle  -1, -1, -3\rangle \text{ m/s/s} , 
\]
\[
     {\bf v}_2 = \langle  -1, 2, 1\rangle \text{ m/s} , 
\]
and
\[
 {\bf p}_2 = \langle  2, 1, 4\rangle \text{ m} . 
\]

Find all times when the rock is slowing down. 

\item Activate the folders \emph{Resolution of Acceleration Vector} in Line 12 and \emph{Graphs, speed function, rate of change of speed function} in Line 17. Reconcile the graphs with the motion and the tangential component of acceleration.

\item Back to working in general, prove that the motion lies in a plane and find an equation of this plane.

\item Find an equation of the specific plane of motion in the animation above. Enter this equation in the worksheet to see if you are correct.

\end{enumerate}
\end{exercise}


\begin{exercise} \label{ExLdf3e3rr3}
...
\item The projectile is slowing down if and only if the angle between the velocity and acceleration vectors is obtuse. This implies the projectile is slowing down if and only if
\[
  {\bf v}(t) \cdot {\bf g} < 0 .
\]
Since 
\begin{align*}
   {\bf v}(t)  &= \frac{d}{dt} \left( {\bf p}(t)  \right)  \\
                  &= \frac{d}{dt} \left( {\bf p}(0) + {\bf v}(0)t + \frac{1}{2}{\bf g}t^2  \right)  \\ 
                  &= {\bf v}(0) + {\bf g} t,
\end{align*}
we know
\begin{align*}
  {\bf v}(t) \cdot {\bf g} &= \left(   {\bf v}(0) + {\bf g} t   \right) \cdot {\bf g} \\
                                    &= {\bf v}(0) \cdot {\bf g} + t {\bf g}  \cdot {\bf g} .
\end{align*}
So the slowing-down condition is that
\[
  {\bf v}(0) \cdot {\bf g} + t {\bf g}  \cdot {\bf g} < 0 .
\]
Solving this inequality for $t$ (and noting that ${\bf g}\cdot {\bf g}>0$) tells us the projectile is slowing down if and only if
\[
     t < - \frac{{\bf v}(0) \cdot {\bf g}}{{\bf g}  \cdot {\bf g}} .
\]

\item The projectile is speeding up when the angle between the acceleration and velocity vectors is acute. This happens exactly when
\[
     t > - \frac{{\bf v}(0) \cdot {\bf g}}{{\bf g}  \cdot {\bf g}} .
\]

\item At the vertex of its parbolic path, the velocity and acceleration vectors are perpendicular (and the speed is a minimum). This happens when ${\bf a}\cdot {\bf v} = 0$ or when
\[
       t_0 = - \frac{{\bf v}(0) \cdot {\bf g}}{{\bf g}  \cdot {\bf g}} .
\]

At this time the projectile has position
\begin{align*}
      {\bf p}(t_0)  &= {\bf p}(0) + {\bf v}(0)t_0 + \frac{1}{2}{\bf g}t_0^2  \\
                         &=  {\bf p}(0) -  \left( \frac{{\bf v}(0) \cdot {\bf g}}{{\bf g}  \cdot {\bf g}}  \right) {\bf v}(0) +\frac{1}{2}\left( \frac{{\bf v}(0) \cdot {\bf g}}{{\bf g}  \cdot {\bf g}}  \right)^2 {\bf g} .
 \end{align*}

\item Since the axis of symmetry is parallel to ${\bf g}$, we can parameterize the axis of symmetry as
\[
 {\bf p}_2(t) = {\bf p}(0) -  \left( \frac{{\bf v}(0) \cdot {\bf g}}{{\bf g}  \cdot {\bf g}}  \right) {\bf v}(0) +\frac{1}{2}\left( \frac{{\bf v}(0) \cdot {\bf g}}{{\bf g}  \cdot {\bf g}}  \right)^2 {\bf g} + t {\bf g} \, , \, t\in \mathbb{R}.
\] 

\item Skip this for now. 

\item The path lies in the plane through the point with position ${\bf p}(0)$ (relative to the origin) that is parallel to the vectors ${\bf v}(0)$ and ${\bf g}$.

So a vector normal to the plane of the path is
\[
 {\bf n} = {\bf v}(0) \times {\bf g} .
\]
So a point $P$ with coordinates $(x,y,z)$ lies in the plane of the path if and only if
\[
  \left(  \langle x, y, z \rangle - {\bf p}(0)   \right) \cdot (   {\bf v}(0) \times {\bf g} ) = 0.
\]
