...
\section*{Spheres}
A sphere is the set of points in space that are all a given distance from a given point. The equation of a sphere is a consequence of this definition and the most important idea is to understand the logic behind the equation.

\begin{question}   \label{QDfsdgfte}
For example, a point $P$ lies on the sphere of radius $5$ centered at the point $A$ with coordinates $(3,1,-2)$ if and only if the distance $\text{dist}(A,P)$ between $P$ and $A$ is equal to $5$. So if $P$ has coordinates $(x,y,z)$ then $P$ lies on this sphere if and only if
\[
   \text{dist}(A,P) = 5 ,
\]
or if and only if 
