...
\begin{exploration}   \label{Exstd67g:Line}
...
\begin{question} \label{Qf4t55lhlnm4}
\begin{enumerate}
...
\begin{question} \label{QLfd3993}
\begin{enumerate}
\item Parameterize the line segment $\overline{AB}$ with endpoints $A(-2,1,3)$ and $B(1,-1,-1)$.

\item Use your parameterization to find the coordinates of the point on the segment $3.5$ times as far from $A$ as from $B$.

\end{enumerate}
\end{question}


\begin{example} \label{Ex343tt4523093}
\begin{enumerate}
...
\begin{question} \label{QLFKdfeMR}
\begin{enumerate}
...
\section{Shadow of a Hoop}

\begin{question}  \label{QLFeereerfer33r}
Imagine a hula hoop like the green one in the worksheet below. We wish to parameterize its shadow on the ground (the $xy$-plane below).

This particular hoop has center $(0,0,3)$ meters and radius $b=2$ meters, but we'll work in general with center $(0,0,a)$ and radius $b$.
...
\begin{onlineOnly}
    \begin{center}
\desmosThreeD{zungfwwq7r}{900}{600}
\end{center}
\end{onlineOnly}

\href{https://www.desmos.com/3d/zungfwwq7r}{163: Parallel Projection}


\begin{enumerate}
\item We'll start by using trigonometric functions to paramaterize the hoop in terms of the marked polar angle $\theta$.

The vector $\overrightarrow{AP}$ from the center of the hoop to the point $P$ on the hoop with polar angle $\theta$ (as marked)  has components
\[
 \overrightarrow{AP} = \langle \answer{0}, \answer{b\cos\theta}, \answer{b\sin \theta}  \rangle. 
\]

The vector $\overrightarrow{OA}$ from the origin to $A$ has components
\[
   \overrightarrow{OA} = \langle \answer{0}, \answer{0}, \answer{a}  \rangle.
\]
So the vector $\overrightarrow{OP}$ has components
\begin{align*}
  \overrightarrow{OP} &= \overrightarrow{OA} + \overrightarrow{AP}  \\
                                 &= \langle  \answer{0} , \answer{b\cos\theta}, \answer{a+b\sin\theta}  \rangle \, , \, 0\leq \theta \leq 2\pi. 
\end{align*}

\item Next we'll parameterize the line through $P$ parallel to the vector ${\bf v}$. 

The point $Q$ with coordinates $(x,y,z)$ lies on this line if and only if
\[
     \langle x,y,z \rangle = \langle 0, b\cos\theta, a+b\sin\theta \rangle + t \langle \answer{v_1}, \answer{v_2}, \answer{v_3} \rangle \text{ for some } t\in \mathbb{R}.
\]

\item The line interects the $xy$-plane when $z=\answer{0}$. Solving for the parameter gives
\[
    t = \answer{\frac{-a-b\sin\theta}{v_3}} .
\] 

\item So the shadow of the point $P$ in the $xy$-plane has coordinates
\[
  (x,y,z) = \left( \answer{-\frac{(a+b\sin\theta)v_1}{v_3}} , \answer{b\cos\theta -  \frac{(a+b\sin\theta)v_2}{v_3}} , 0\right) .
\]
\end{enumerate}
\end{question}
...
\begin{question}  \label{QLFeereerf3433r}
This is a variation of the previous question where we compare the speeds of a point moving around the hula hoop with the speed of its shadow in the $xy$-plane. %Imagine a hula hoop like the green one in the worksheet below. We wish to parameterize its shadow on the ground (the $xy$-plane below).

As before, the hoop has center $(0,0,3)$ meters and radius $b=2$ meters, but we'll work in general with center $(0,0,a)$ and radius $b$.
...
\begin{onlineOnly}
    \begin{center}
\desmosThreeD{zv4pchit0m}{900}{600}
\end{center}
\end{onlineOnly}

\href{https://www.desmos.com/3d/zv4pchit0m}{163: Parallel Projection}


\begin{enumerate}
\item Play the slider $t_1$ in Line 2 of the worksheet to make the green point $P$ move around the hoop at a constant speed $v$, measured in cm/sec.

\item Describe how the speed of its shadow $Q$ (blue) varies as $P$ moves around the hoop.

\item What does the speed of $Q$ seem to depend upon?

\item Parameterize the motion of $P$ around the hoop and the motion of its shadow $Q$.

\item Express the speed of $Q$ in terms of the speed of $P$ and the marked angle $\theta$.
\end{enumerate}
\end{question}


\section{Lamp Shade Shadow}

\begin{question}  \label{QLFeefer33r}

\begin{onlineOnly}
    \begin{center}
\desmosThreeD{oqrd2iy7r7}{900}{600}
\end{center}
\end{onlineOnly}

\href{https://www.desmos.com/3d/oqrd2iy7r7}{163: Light Shadow}
