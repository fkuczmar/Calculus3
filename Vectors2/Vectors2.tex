...
Suppose, for example, we want to find an equation of the circle of radius $5$ centered at the point $A$ with coordinates $(4,1)$. Then with $O$ as the origin, the point $P(x,y)$ with coordinates lies on this circle if and only if the length of the vector 
\begin{align*}
...
It's important to realize that even though we're thinking of vectors when we write this so-called \emph{vector equation}, the circle is still a \emph{set of points}, namely the set
\[
   \{ (x,y) \in \mathbb{R}^2\, | ,   \left|   \langle x,y \rangle - \langle 4, 1 \rangle   \right| = 5 \}.
\]
It is \emph{not} a set of vectors.
...
This way of thinking about vectors when writing an equation of a circle is especially handy in desmos. With $A=(4,1)$ as the center of the circle, Line 2 of the worksheet below shows how to write the vector equation of the circle in desmos.
