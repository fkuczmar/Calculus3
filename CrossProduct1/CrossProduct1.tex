...
\begin{question}  \label{QODFOerreed}
Given points $A$, $B$ in $\mathbb{R}^3$ not colinear with the origin $O$, our aim is to construct a square $ABCD$ lying in the plane through $A$, $B$, and $O$.

By this we mean to express the vectors $\overrightarrow{OC}$ and  $\overrightarrow{OD}$ in terms of the vectors $\overrightarrow{OA}$ and $\overrightarrow{OB}$.

\begin{enumerate}

\item How many possibilites are there for the vertex $C$ of our square? Remember that $A$, $B$, $C$, $D$ are consecutive vertices of square $ABCD$.
\begin{multipleChoice}
\choice{none}
\choice{one}
\choice[correct]{two}
...
\begin{onlineOnly}
    \begin{center}
\desmosThreeD{awowygd8ks}{900}{600}
\end{center}
\end{onlineOnly}

\href{https://www.desmos.com/3d/awowygd8ks}{163: Square 44B}   %vyzghwolop



\item To get started, first find an expression for a vector ${\bf n}$ normal to the plane through $A$, $B$, and $O$. Work in general, not with the specific points $A$ and $B$ in the above workseet. 

\item Enter your expression for the normal vector in Line 4 above. Type \emph{cross} for the cross product. Remember also to omit the $O's$ in your vectors. For example, write $\overrightarrow{OA}$ as $A$.

\item Find a vector equation for the plane through $O$, $A$, and $B$. 

The key idea is that a point $P$ with coordinates $(x,y,z)$ lies in this plane if and only if the vector $\overrightarrow{OP}$ is normal (ie. perpendicular) to the vector ${\bf n}$ normal to the plane. So $P(x,y,z)$ lies in the plane if and only if
\[
   \overrightarrow{OP} \cdot {\bf n} = \answer{0}
\]
or if and only if
\[
  \langle x, y, z \rangle \cdot {\bf n} = \answer{0}.
\]

\item Enter your equation for the plane in Line 5 above. The scalar product in desmos is the * key, above the number $8$.

\item Now to complete the square, we need a vector ${\bf m}$ 
\begin{enumerate}
\item perpendicular to the vector $\overrightarrow{AB}$,

\item parallel to the plane (ie. normal to ${\bf n}$), and

\item having the same length as $\overrightarrow{AB}$.
\end{enumerate}

Find an expression for such a vector ${\bf m}$ (there are two possibilities). Enter  your expression in Line 6.

\item Now use vector addition to express the vector $\overrightarrow{OC}$ from the origin to the third vertex of square $ABCD$ in terms of $\overrightarrow{OA}$, $\overrightarrow{OB}$ and ${\bf m}$. Enter this expression in Line 7.

\item Use vector addition again to find a similar expression for $\overrightarrow{OD}$. Enter this in Line 8.

\item Finally, parameterize the sides $\overline{BC}$, $\overline{CD}$, and $\overline{DA}$ of the square. Enter these parameterizations in Lines 9-11. Don't forget to include the correct domains.

\item Then construct the second square similarly.
...
This problem is similar to the previous one. The only difference is that now we are given a pair of \emph{opposite} vertices $A$ and $C$ of square $ABCD$ lying in the plane through the non-colinear points $A$, $B$, and $P$.

\begin{enumerate}
\item{ How many possible squares $ABCD$ lie in a given plane and have given points $A$ and $C$ as two of its opposite vertices? 
\begin{multipleChoice}
\choice{none}
\choice[correct]{one}
\choice{two}
...
\item{Express the vector $\overrightarrow{OB}$ from the origin to $B$ in terms of the vectors $\overrightarrow{OA}$, $\overrightarrow{OC}$, and 
\[
   {\bf n} = ( {\bf p}-{\bf a}  ) \times ({\bf p}-{\bf c})  .
\] 
...
\]
There are two possibilites. Find both.
}

\item{Enter one of your expressions for $\overrightarrow{OC}$ on Line 16 of the desmos worksheet below to check your work.}
...
\item{Enter one of your expressions for $\overrightarrow{OD}$ on Line 17 of the desmos worksheet below to check your work.}

\end{enumerate}

\begin{onlineOnly}
    \begin{center}
\desmosThreeD{2yonxicrjl}{900}{600}
\end{center}
\end{onlineOnly}

\href{https://www.desmos.com/3d/2yonxicrjl}{163: Equilateral Triangle 2}
